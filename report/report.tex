...
The number of samples to create near the obstacles was fixed based on previous experiments, so that we have enough points to try over all the iteration but we do not compute too many uselessly. Thus we chose \texttt{num_points_near_obstacles = 1000}. 
The probabilities $\texttt{p\_v}$ and $\texttt{p\_e}$ were chosen to efficiently target corners, which are key points for efficient paths. However, setting $\texttt{p\_v}$ too high is not relevant, as this would create too much branches of the tree directed towards uninteresting corners. That is why we chose $\texttt{p\_v = 0.4}$. It is the same idea for edge sampling : if the probability is too low, we do not focus enough on edges. If it is too high, we overfocus on some edges, even though they are irrelevant for our problem. That is why we chose $\texttt{p\_e = 0.3}$, so that we have a fairly exhaustive exploration of edges without trying every precomputed point.
...
The idea is to find a path for the first robot, and then to build a new tree following RRT rules with a new constraint : the new tree should not cut the path of the first robot. To prevent the first robot to take a path too close from obstacles, which would impede the second robot to find his way to the goal point, the problem data given to the first robot is quite different : all obstacles are enlarged of $3R$ in each direction, so that there remains enough space for the second robot to go between the first one's path and the real obstacle.
