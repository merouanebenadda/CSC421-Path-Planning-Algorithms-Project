...
\SetAlgoLined
\caption{AddVertex}
\KwIn{Point $v$, index $parent\_index$}
Ajouter $v$ à $tree.vertices$\;
Ajouter $parent\_index$ à $tree.parents$\;
$cost \gets tree.costs[parent\_index] + d(tree.vertices[parent\_index], v)$\;
Ajouter $cost$ à $tree.costs$\;
\end{algorithm}

\begin{algorithm}[H]
\SetAlgoLined
\caption{ReconstructPath}
\KwIn{Index $vertex\_index$}
\KwOut{Chemin de points}
$path \gets \emptyset$\;
$current \gets tree.parents[vertex\_index]$\tcp*{Partir du parent du goal}
\While{$tree.parents[current] \neq -1$}{
    Ajouter $tree.vertices[current]$ à $path$\;
    $current \gets tree.parents[current]$\;
}
Inverser $path$\tcp*{Obtenir le chemin de start à goal}
\Return $path$\;
\end{algorithm}

\begin{algorithm}[H]
\SetAlgoLined
\caption{BuildRRT (variante RRT*)}
\KwIn{Problème $problem$, distance $\delta_s$, rayon $\delta_r$, entier $max\_iterations$}

$iterations \gets 0$\;
\While{$iterations < max\_iterations$}{
    \tcp{Échantillonner un point aléatoire}
    $v_r \gets$ point aléatoire dans $[0, x_{max}] \times [0, y_{max}]$\;
    
    \If{$v_r$ est dans un obstacle}{
        \textbf{continuer}\;
    }
    
    \tcp{Trouver le sommet le plus proche}
    $v_n \gets \argmin_{v \in tree.vertices} d(v, v_r)$\;
    
    \tcp{Créer un nouveau point vers $v_r$ à distance max $\delta_s$}
    \eIf{$d(v_n, v_r) \leq \delta_s$}{
        $v \gets v_r$\;
    }{
        $\theta \gets \arctan2(v_r.y - v_n.y, v_r.x - v_n.x)$\;
        $v.x \gets v_n.x + \delta_s \cos(\theta)$\;
        $v.y \gets v_n.y + \delta_s \sin(\theta)$\;
    }
    
    \tcp{Choisir le meilleur parent dans le rayon $\delta_r$ (RRT*)}
    $parent \gets$ NULL\;
    \If{pas de collision entre $v_n$ et $v$}{
        $parent \gets v_n$\;
    }
    
    \ForEach{$v_i \in tree.vertices$}{
        \If{$d(v_i, v) < \delta_r$ \textbf{et} pas de collision entre $v_i$ et $v$}{
            $cost_{new} \gets tree.costs[v_i] + d(v_i, v)$\;
            \If{$parent =$ NULL \textbf{ou} $cost_{new} < tree.costs[parent] + d(parent, v)$}{
                $parent \gets v_i$\;
            }
        }
    }
    ...
    \If{$parent =$ NULL}{
        \textbf{continuer}\tcp*{Aucun parent valide trouvé}
    }
    
    \textsc{AddVertex}$(v, parent)$\;
    $idx_v \gets |tree.vertices| - 1$\;
    
    \tcp{Rewiring : recâbler les voisins (RRT*)}
    \For{$i \gets 0$ \KwTo $|tree.vertices| - 1$}{
        \If{$d(tree.vertices[i], v) < \delta_r$ \textbf{et} pas de collision}{
            $cost_{via\_v} \gets tree.costs[idx_v] + d(v, tree.vertices[i])$\;
            \If{$tree.costs[i] > cost_{via\_v}$}{
                $tree.parents[i] \gets idx_v$\;
                $tree.costs[i] \gets cost_{via\_v}$\;
            }
        }
    }
    
    \tcp{Vérifier si le goal est atteignable}
    \If{$d(v, goal) \leq \delta_s$ \textbf{et} pas de collision entre $v$ et $goal$}{
        \textsc{AddVertex}$(goal, idx_v)$\;
        \textbf{sortir}\tcp*{Goal atteint}
    }
...
\begin{algorithm}[H]
\SetAlgoLined
\caption{RRTPath - Planification de chemin}
\KwIn{Problème $problem$, distance $\delta_s$, rayon $\delta_r$, entier $max\_iterations$}
\KwOut{Chemin de points}

\textsc{BuildRRT}$(problem, \delta_s, \delta_r, max\_iterations)$\;
$goal\_index \gets |tree.vertices| - 1$\tcp*{Le goal est le dernier sommet}
